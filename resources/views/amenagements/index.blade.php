...
@section('nav-title', 'AMENAGEMENT DANS LA COMMUNAUTE')
...
@section('content')
                            ...
                            @foreach ($allRefuse as $amenagee)
                                <li class="d-flex mb-4 pb-1">
                                    ...
                                    <div class="d-flex w-100 flex-wrap align-items-center justify-content-between gap-2">
                                        <div class="me-2">
                                            <h6 class="mb-0"><b>
                                                    Aménagement de</b>&nbsp;&nbsp;{{ $amenagee->nom_complet_amg }}</h6>
                                            <small class="text-muted">Envoyé par
                                                @if ($amenagee->individus_sended_id === null)
                                                    Administrateur(vous)
                                                @else
                                                    {{ $amenagee->IndividusSendAmenagement->nom_individu }}&nbsp;{{ $amenagee->IndividusSendAmenagement->prenom_individu }}
                                                @endif
                                            </small>
                                        </div>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                @if ($amenagee->status === 0 || $amenagee->status === 2)
                                                    <form action="{{ route('amenagement.update', [$amenagee]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                ...
                                                @endif
                                                @if ($amenagee->status === 0 || $amenagee->status === 1 || $amenagee->status === 2)
                                                    <form action="{{ route('amenagement.update', [$amenagee]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
    ...
    <div class="modal applyLoanModal fade" id="ModalListeSupprime" tabindex="-1"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalScrollableTitle" style='font-weight: 900;'>Demande d'aménagement
                        supprimée</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    @if (count($allSupprime) > 0)
                        <ul class="p-0 m-0">
                            @foreach ($allSupprime as $amenagee)
                                <li class="d-flex mb-4 pb-1">
                                    <div class="avatar flex-shrink-0 me-3">
                                        <span class="avatar-initial rounded bg-label-danger"><i
                                                class="fa fa-car"></i></span>
    ...
    {{-- DETAILS AMENAGEMENTS ENREGISTRES --}}
    @foreach ($amenagements->sortByDesc('created_at') as $amenage)
        <x-details-amenagement :amenagement="$amenage" />
    @endforeach
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            <h5 class="card-title">TABLE DES AMENAGEMENTS</h5>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-end" aria-labelledby="transactionID" style="">
                    <a class="dropdown-item" href="{{ route('amenagement.create') }}">Nouveau aménagement</a>
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th style="font-weight: bold;">Vit chez</th>
                    <th style="font-weight: bold;">Date d'aménagement</th>
                    <th style="font-weight: bold;">Provenance</th>
            ...
            <tbody class="table-border-bottom-0">
                @if ($amenagements)
                    @foreach ($amenagements as $amenagee)
                        <tr>
                            <td>{{ $amenagee->nom_complet_amg }}</td>
                            <td>{{ $amenagee->FamilleAmenagement->nom_famille }}</td>
                            <td>{{ $amenagee->date_amenagement }}</td>
                            <td>{{ $amenagee->provenance }}</td>
                            <td>
                                @if ($amenagee->status === 0)
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" type="button" data-bs-toggle="modal"
                                            data-bs-target="#details_amenagement_{{ $amenage->id }}">
                                            <i class="bx bx-show me-1"></i>
                                            Voir details
                                        </a>
                                        @if ($amenagee->status === 0 || $amenagee->status === 2)
                                            <form action="{{ route('amenagement.update', [$amenagee]) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <input type="hidden" name="status" value="1">
                                                <button type="submit" class="dropdown-item"
                                        ...
                                        @if ($amenagee->status === 0 || $amenagee->status === 1)
                                            <form action="{{ route('amenagement.update', [$amenagee]) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <input type="hidden" name="status" value="2">
                                                <button type="submit" class="dropdown-item"
                                                    onclick="return confirm('Confirmez-vous votre choix ?');">
